...
def plot_stacked_histogram(sim_folder, data_folder, sim_samples, variables, out_path, bins=40, mass_window=(120, 130), signal_scale=100):
    ...
    variables = variables + class_names[:num_classes]

    var_config = {
        "mass": {"label": r"$m_{\gamma\gamma}$ [GeV]", "bins": 80, "range": (100, 180), "log": True},
        "nonRes_dijet_mass": {"label": r"$m_{jj}$ [GeV]", "bins": 80, "range": (70, 190), "log": True},
        "non_resonant_bkg_score": {"label": "non_resonant_bkg_score", "bins": 60, "range": (0, 1), "log": True},
        "ttH_score": {"label": "ttH_score", "bins": 60, "range": (0, 1), "log": True},
        "other_single_H_score": {"label": "other_single_H_score", "bins": 60, "range": (0, 1), "log": True},
        "GluGluToHH_score": {"label": "GluGluToHH_score", "bins": 60, "range": (0, 1), "log": True},
        "VBFToHH_sig_score": {"label": "VBFToHH_sig_score", "bins": 60, "range": (0, 1), "log": True}
    }
    ...
    for variable in variables:
        ...
        if var_config[variable]["log"]:
            ax.set_yscale("log")
            ax.set_ylim(0.001, 100 * np.max(data_hist))
        else:
...
if __name__ == "__main__":
    sim_folder = "MLP_multiclass_ang_20250308/individual_samples"
    data_folder = "MLP_multiclass_ang_20250308/individual_samples_data"
    ...
    variables = ["mass", "nonRes_dijet_mass"]
    out_path = "MLP_multiclass_ang_20250308/"
