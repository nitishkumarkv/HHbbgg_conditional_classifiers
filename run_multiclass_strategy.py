...
def perform_training(args):
    ...
    out_path = args.out_path
    train_split_path = f"{out_path}/Train_split"
    final_split_path = f"{out_path}/Final_split"
    config_path = args.config_path
    ...
    if do_random_search:
        ...
        subprocess.run(f"python3 models/random_search.py --input_path {train_split_path} --training_config_path {training_config_path}", shell=True)        

    # perform trainging
    if args.train_best_model:
        print('INFO: Training the best model')
        subprocess.run(f"python3 models/training_utils.py --input_path {train_split_path} --training_config_path {training_config_path}", shell=True)

    # plot the training results
    if args.plot_training_results:
        print('INFO: Getting the results plots')
        subprocess.run(f"python3 models/mlp_plotter.py --input_path {train_split_path} --config_path {config_path}/training_config.yaml", shell=True)
        
    # get permutaion importance
    if args.get_permutation_importance:
        print('INFO: Getting permutation importance')
        subprocess.run(f"python3 models/permutation_importance.py --input_path {train_split_path}", shell=True)
    
    # get the predictions
    if args.get_predictions:
        print('INFO: Getting the predictions nominal')
        subprocess.run(f"python3 models/get_prediction.py --model_folder {train_split_path}/after_random_search_best1/ --samples_path {final_split_path} --config_path {config_path} --get_pred_nominal", shell=True)

    # get the predictions for systematics
    if args.get_predictions_sys:
        print('INFO: Getting the predictions systematics')
        subprocess.run(f"python3 models/get_prediction.py --model_folder {train_split_path}/after_random_search_best1/ --samples_path {final_split_path} --config_path {config_path} --get_pred_sys", shell=True)
    ...
    if args.test_mass_sculpting:
        ...
        subprocess.run(f"python3 utils/test_cor_mass.py --input_path {final_split_path} --config_path {config_path}", shell=True)

    # get the predictions for data
    if args.get_data_mc_plots:
        print('INFO: Getting data-MC plots')
        subprocess.run(f"python3 utils/plotting_utils.py --base-path {final_split_path} --training_config_path {training_config_path}", shell=True)

    # get score shapes for different kl samples
    if args.get_score_shape_diff_kl:
        print('INFO: Getting score shape differences')
        subprocess.run(f"python3 utils/score_shape_diff_kl.py --folder {final_split_path}/individual_samples/", shell=True)
