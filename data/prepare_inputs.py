...
class PrepareInputs:
    def __init__(
        ...
        if self.training_info is not None:
            ...
            self.data_prep_split = self.training_info.get("data_prep_split", None)
        else:
    ...
    def _get_split_paths(self):
        if self.data_prep_split is None:
            return self.outpath, self.outpath
        train_split_name = self.data_prep_split.get("train_split_name", "Train_split")
        final_split_name = self.data_prep_split.get("final_split_name", "Final_split")
        return f"{self.outpath}/{train_split_name}", f"{self.outpath}/{final_split_name}"


    def _copy_training_files_to_final(self, train_path, final_path):
        if self.data_prep_split is None:
            return
        os.makedirs(final_path, exist_ok=True)
        for file_name in ["mean_std_dict.pkl", "input_vars.txt"]:
    ...
    def _data_prep_train_split_fraction(self):
        if self.data_prep_split is None:
            return 1.0
        return self.data_prep_split.get("train_split_fraction", 0.5)


    def _data_prep_split_scales(self):
        if self.data_prep_split is None:
            return 1.0, 1.0
        train_fraction = self._data_prep_train_split_fraction()
    ...
    def prep_inputs_for_training(self):
        ...
        out_path, final_path = self._get_split_paths()
                ...
                if self.data_prep_split is not None:
                    ...
                    for file_path, xsec_name in file_xsec_pairs:
                        for batch in self._iter_file_batched(
                            ...
                            train_idx, _ = self._data_prep_split_indices(len(batch))
                            train_weight_scale, _ = self._data_prep_split_scales()
                            train_events = self._scale_event_weights(batch[train_idx], train_weight_scale)
                            train_events = self.preselection(train_events)
                            if len(train_events) > 0:
                                events_list.append(train_events)
                            del batch, train_events
                    if not events_list:
        ...
        self._copy_training_files_to_final(out_path, final_path)
    ...
    def prep_inputs_for_prediction_sim(self):
        ...
        inputs_path, final_inputs_path = self._get_split_paths()
            ...
            for samples in training_info["samples_info"][era].keys():
                ...
                for file_path, xsec_name in file_xsec_pairs:
                    for batch in self._iter_file_batched(
                        ...
                        train_idx, final_idx = self._data_prep_split_indices(len(X_chunks[-1]))
                        train_weight_scale, final_weight_scale = self._data_prep_split_scales()
                        X_chunks[-1] = X_chunks[-1][train_idx]
                        w_chunks[-1] = w_chunks[-1][train_idx] * train_weight_scale
                        split_info = [
                            (train_idx, parquet_chunks, full_path_to_save, "train", train_weight_scale),
                        ]
                        if final_idx is not None:
                            X_batch_final = np.column_stack([
                                ak.to_numpy(ak.fill_none(batch[var], -999.0))
                                for var in vars_for_training
                            ]).astype(np.float32)[final_idx]
                            w_batch_final = ak.to_numpy(
                                ak.fill_none(batch["rel_xsec_weight"], np.nan)
                            ).astype(np.float32)[final_idx] * final_weight_scale
                            X_chunks_final.append(X_batch_final)
                            w_chunks_final.append(w_batch_final)
                            split_info.append((final_idx, parquet_chunks_final, final_full_path_to_save, "final", final_weight_scale))
                        for idx, parquet_split_chunks, path_to_save, split_name, weight_scale in split_info:
    ...
    def prep_inputs_for_prediction_data(self):
        ...
        if self.data_prep_split is not None:
            out_path = f"{final_inputs_path}/individual_samples_data/"
        os.makedirs(out_path, exist_ok=True)
