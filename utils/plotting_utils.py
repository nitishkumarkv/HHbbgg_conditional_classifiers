...
"""def plot_stacked_histogram(samples_info, sim_folder, data_folder, sim_samples, variables, out_path, bins=40, mass_window=(120, 130), mjj_mass_window=(110, 140), signal_scale=100, only_MC=False, only_blind_mass_variable=True):
    
    # create output directory if it does not exist
    if only_MC:
        out_path = os.path.join(out_path, "MC_Plots_2022")
    else:
        out_path = os.path.join(out_path, "Data_MC_Plots_2022")
    ...
    #                               & ((data_combined["Res_mjj_regressed"] < mjj_mass_window[0]) | (data_combined["Res_mjj_regressed"] > mjj_mass_window[1]))]
    if not only_blind_mass_variable:
        data_combined = data_combined[((data_combined["mass"] < mass_window[0]) | (data_combined["mass"] > mass_window[1]))]
    ...
    for sample in sim_samples:
        sample_combined = []
        for era in eras:
            events_ = ak.from_parquet(f"{events_path}/{samples_info[era][sample]}", columns=variables+["lead_isScEtaEB", "lead_isScEtaEE", "sublead_isScEtaEB", "sublead_isScEtaEE"])
    ...
    # 1. Accumulate total MC weight in sideband
    if not only_blind_mass_variable:
        total_mc_weight_sideband = 0

        for sample, data in stack_mc_dict.items():
            #sideband_cut = (
            #    (data["mass"] < mass_window[0]) | (data["mass"] > mass_window[1])
            #) & (
            #    (data["Res_mjj_regressed"] < mjj_mass_window[0]) | (data["Res_mjj_regressed"] > mjj_mass_window[1])
            #)
            #total_mc_weight_sideband += ak.sum(data["weight_tot"][sideband_cut])

            sideband_cut = (data["mass"] < mass_window[0]) | (data["mass"] > mass_window[1])

            #total_mc_weight_sideband += ak.sum(data["weight_tot"][sideband_cut])
            total_mc_weight_sideband += ak.sum(data["weight_tot"])

        print(f"Total MC weight in sideband: {total_mc_weight_sideband:.3f}")

        # 2. Normalization factor
        mc_norm_factor = int_data_sideband / total_mc_weight_sideband
        print(f"Scaling all MC weights by: {mc_norm_factor:.3f}")

        for sample in stack_mc_dict:
            weight = stack_mc_dict[sample]["weight_tot"]
        stack_mc_dict[sample]["weight_tot"] = weight * mc_norm_factor
