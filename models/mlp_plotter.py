        ...
        if len(samples_in_class) > 0:
            ...
            for class_plot_idx, (json_class_name, model_class_name) in enumerate(json_class_to_model_class.items()):
                if model_class_name not in class_names:
                    print(f"WARNING: Mapped class '{model_class_name}' is missing from class_names. Skipping this class.")
                    continue

                score_idx = class_names.index(model_class_name)
                fig, ax = plt.subplots(figsize=(8, 6))
                max_y = 0
                ...
                fig.tight_layout()
                fig.savefig(f'{path_for_plots}/gghh_all_samples_{model_class_name}_score.png', dpi=150)
                plt.close(fig)

            print("INFO: Class-wise all-sample score plots completed!")
        else:
            print("WARNING: No samples found for class 'is_GluGluToHH_sig'. Skipping per-sample all-class plots.")
