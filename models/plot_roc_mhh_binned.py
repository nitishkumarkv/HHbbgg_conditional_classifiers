...
This script reads:
- mhh variable, sample names, and weights from parquet files (individual_samples/{era}/{sample}/events.parquet)
- Predictions from y.npy in the same directory
- Class labels from sample_to_class_mapping.json

Then generates ROC curves for each mhh bin, with automatic Run2/Run3 separation.

Usage:
    python plot_roc_mhh_binned.py \
        --base_path <path_to_inputs> \
...
def load_mhh_y_from_parquet(base_path, mhh_var_name):
    """
    Load mhh variable, predictions, labels, weights, and era labels from parquet and y.npy files.
    
    Args:
        base_path: Base path to prepared inputs with individual_samples folder
    ...
    # Load sample_to_class_mapping.json
    ...
    parquet_files = sorted(glob.glob(f"{base_path}/individual_samples/*/*/events.parquet"))
    
    if len(parquet_files) == 0:
        raise ValueError(f"No parquet files found in {base_path}/individual_samples/*/*/events.parquet")
