...
from gato_utils import (
    ...
    plot_yield_histories, make_schedule,
)
...
"""def load_data_samples(base_path, data_list):

    # loop over all the data_list and return a single dataframe with all the data
    data_frames = []
    dijet_mass_key = "nonResReg_vbfpair_dijet_mass_DNNreg"
    for data in data_list:
        data_dir = os.path.join(base_path, "individual_samples_data/", data)
        evt_file = os.path.join(data_dir, "events.parquet")
        y_file = os.path.join(data_dir, "y.npy")

        df = pd.read_parquet(evt_file, columns=[
                            "mass", dijet_mass_key,
                            "weight_tot",
                            "lead_mvaID", "sublead_mvaID",
                        ])
...
def load_data_samples(base_path, data_list, n_workers=1, dim=3):
...
class GMMMultiDim(gato_gmm_model):
    ...
    * signal_pattern : substring to identify the signal process in data_dict keys
    ...
    def call(self, data_dict, rewt=None):
        ...
        S_ttH = tf.zeros(self.n_cats, dtype=tf.float32)
        S_tH  = tf.zeros(self.n_cats, dtype=tf.float32)
        ...
        for proc, t in data_dict.items():
            ...
            if proc.startswith("GluGlutoHHto2B2G_kl_1p00_kt_1p00_c2_0p00"):
                S_ttH += yields
            elif proc.startswith("VBFHH_CV_1p000_C2V_1p000_C3_1p000"):
                S_tH  += yields
            else:
                B     += yields * fac
                B_w2  += sumw2 * fac
        ...
        Z_ttH = tf.sqrt(tf.reduce_sum(asymptotic_significance(S_ttH, B + S_tH)**2))
        Z_tH  = tf.sqrt(tf.reduce_sum(asymptotic_significance(S_tH,  B + S_ttH)**2))

        # geometric mean loss (negative, to maximise)
        loss = -tf.sqrt(Z_ttH * Z_tH)

        return loss, B, nonres, S_ttH, S_tH, B_w2, Z_ttH, Z_tH
...
def train_step(model, tensor_data, opt, rewt, lam=0.0, penalty_threshold=10.0):
    with tf.GradientTape() as tape:
        significance_loss, B, yield_nonres, S_ttH, S_tH, B_w2, Z_ttH, Z_tH = model.call(tensor_data, rewt)
    ...
    return total_loss, significance_loss, penalty, Z_ttH, Z_tH, B, yield_nonres, S_ttH, S_tH,
...
def main():
    ...
    parser.add_argument("--temp_init", type=float, default=0.5, help="Initial GMM temperature")
    ...
    parser.add_argument("--out_folder", type=str, default="./optimisationOutput/OptimisationGMM_multiclass_cube_new/", help="Base output folder for results")
    parser.add_argument("--input_dim", type=int, default=3, help="Dimensionality of input discriminant vector (default: 3)")
    ...
    path_output_plots = (
        f"./optimisationOutput/OptimisationGMM_multiclass_cube_new/"
        f"{args.out_folder}_{args.n_cats}_cats_lam_{lam_str}_pen_{penalty_str}_opt_{args.optimizer}_epochs_{args.epochs}/"
    )
    ...
    if not os.path.exists(history_filename):
        with open(history_filename, "w", newline="") as csvfile:
            ...
            writer.writerow(["epoch", "z_ttH", "z_tH", "loss"])
    ...
    def append_history(epoch_idx, significance_loss, reg, z_ttH, z_tH, S_ttH, S_tH, B, yield_nonres, *, force_bias=False):
        z_ttH_val = z_ttH.numpy()
        z_tH_val = z_tH.numpy()
        loss_val = significance_loss.numpy()
        z_ttH_history.append(z_ttH_val)
        z_tH_history.append(z_tH_val)
        loss_history.append(loss_val)
        regularisation_history.append(reg.numpy())
        S_ttH_history.append(tf.stack(S_ttH).numpy())
        S_tH_history.append(tf.stack(S_tH).numpy())
        B_history.append(tf.stack(B).numpy())
        B_nonres_history.append(yield_nonres.numpy())

        if epoch_idx % 10 == 0:
            print(
                f"Epoch {epoch_idx}: z_ggHH={z_ttH_val:.3f}, z_vbfHH={z_tH_val:.3f}, "
                f"S_ggHH={S_ttH_history[-1]}, S_vbfHH={S_tH_history[-1]}, B={B_history[-1]}"
            )
        ...
        with open(history_filename, "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([epoch_idx, z_ttH_val, z_tH_val, loss_val])
    ...
    if optimizer_name in ("adam", "rmsprop"):
        ...
        for epoch in range(args.epochs):
            ...
            _, significance_loss, reg, z_ttH, z_tH, B, yield_nonres, S_ttH, S_tH = train_step(
                model,
                tensor_data,
                opt,
                rewt=factors_tf,
            ...
            append_history(epoch, significance_loss, reg, z_ttH, z_tH, S_ttH, S_tH, B, yield_nonres, force_bias=(epoch == args.epochs - 1))
        ...
        for epoch in range(warmup_epochs):
            ...
            update_best(float(significance_loss.numpy()), epoch)
            temp_sched.update(epoch)
            append_history(epoch, significance_loss, reg, z_ttH, z_tH, S_ttH, S_tH, B, yield_nonres, force_bias=(epoch == warmup_epochs - 1))
        ...
        def loss_and_grad(weights):
            ...
            with tf.GradientTape() as tape:
                ...
                sig_loss, B, yield_nonres, S_ttH, S_tH, B2, Z_ttH, Z_tH = loss_vals
                penalty = low_bkg_penalty(yield_nonres, threshold=penalty_threshold)
            ...
            if (eval_count % 5) == 0:
                print(
                    ...
                    f"Z_ttH={float(Z_ttH.numpy()):.3f} Z_tH={float(Z_tH.numpy()):.3f}"
                )

            append_history(eval_count, sig_loss, penalty, Z_ttH, Z_tH, S_ttH, S_tH, B, yield_nonres)
            eval_count += 1
    ...
    plot_significance_and_loss_histories(
        path_output_plots, z_ttH_history, z_tH_history, loss_history, regularisation_history
    )
    ...
    # Yield histories
    epochs_arr = np.arange(len(z_ttH_history))
    plot_yield_histories(
        path_output_plots, epochs_arr,
        S_ttH_history, S_tH_history, B_nonres_history,
